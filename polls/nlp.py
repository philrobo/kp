...
class chat(object):
    ...
    global path
    path = os.path.join(my_path, "nlp")
    # self.response(chat_msg)

    def __init__(self, chat_msg):
        print("path is geting printed..........."+path)
        self.input = chat_msg
    ...
    def classify(self, sentence):
        ...
        net = tflearn.fully_connected(
            net, len(train_y[0]), activation='softmax')
        net = tflearn.regression(net)
        model = tflearn.DNN(net, tensorboard_dir='tflearn_logs')
        model.load(
            path+'/newmodel2.tflearn')
    ...
    def response(self, sentence, userID='123', show_details=True):
        ...
        with open(path + '/intents.json') as json_data:
